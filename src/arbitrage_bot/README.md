...
Workflow:
  ...
  - 3) Return a tuple of floats:
     - `p_diff` as a decimal fraction. For example, 0.05 means 5%.
     - `price_reference` which is the price reference according to the order type
...
---
### 4. `_enforce_minimum_amounts(sub_orders: List[Dict[str, Any]], side: str) -> Any`
...
---
### 5. `split_order_into_suborders(order: ArbitrageOrder, reference_price: float, side: str, delta: Optional[float] = None) -> Any`
...
---
### 6. `place_sub_orders(sub_orders: List[Dict[str, Any]], arb_order: ArbitrageOrder) -> Union[List[Dict[str, Any]], Dict[str, Any]]`
...
### 7. `place_sub_order_cancellations(sub_orders: List[Dict[str, Any]], arb_order: ArbitrageOrder) -> List[Dict[str, Any]]`
...
### 8. `_handle_unprepared_or_minimum_amount_errors(sub_order_responses: Union[List[Dict[str, Any]], Dict[str, Any]]) -> Union[None, Dict[str, Any]]`
...
---
### 10. `_wait_for_orders_to_leave_received(received_orders: List[Dict[str, Any]], arb_order: ArbitrageOrder, max_wait_seconds: int = 10) -> None`
...
### 11. `_update_arbitrage_order_on_fill(sub_order_dict: Dict[str, Any], new_state: str, traded_base_amount: float, traded_quote_amount: float, arb_order: ArbitrageOrder) -> None`
...
### 12. `execute_opposite_order_high_liquidity_exchange(arb_order: ArbitrageOrder) -> Union[Dict[str, Any], Dict[str, Any]]`
...
### 15. `_split_btc_amount(total_btc: float) -> List[float]`
...
---
### 16. `quote_currency_transfer(arb_order: ArbitrageOrder) -> bool`
